...
namespace ADS::UI {
    ...
    void Window::setStyle()
    {
        ...
        // Scale widget sizes by the OS DPI preference only.
        // Physical pixel density is a separate concern handled each frame by
        // SDL_SetRenderScale(DisplayFramebufferScale) in App::render().
        float displayScale = SDL_GetWindowDisplayScale(this->window);
        if (displayScale < 1.0f) displayScale = 1.0f;
        // Honour DISPLAY_SCALE env override (mirrors font-loading cascade in main.cpp)
        ...
        this->style->ScaleAllSizes(displayScale);
