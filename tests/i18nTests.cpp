...
{
    return std::find(container.begin(), container.end(), value) != container.
           end();
}
...
{
    return lang == "en_US" || lang == "es_ES" || lang == "fr_FR" || lang ==
           "de_DE";
}
...
{
    if (locale.find("en_US") != std::string::npos) return "en_US";
    if (locale.find("es_ES") != std::string::npos) return "es_ES";
    if (locale.find("fr_FR") != std::string::npos) return "fr_FR";
    if (locale.find("de_DE") != std::string::npos) return "de_DE";
    return "";
}
...
i18nTests::createPropertiesFile(const std::string& lang,
                                const std::unordered_map<
                                    std::string, std::string>& translations)
{
...
void
i18nTests::createJsonFile(const std::string& lang,
...
void
i18nTests::createPoFile(const std::string& lang,
...
void
i18nTests::createMalformedFile(const std::string& name,
...
TEST_F(i18nTests, Success_Object_Initialization)
{
    std::unique_ptr<i18n> i18nObject = SetupI18nObject(
            BASE_FOLDER, SPANISH_SPAIN.data());
...
TEST_F(i18nTests, Failed_Object_Initialization_non_existing_folder)
{
    EXPECT_THROW(
            std::unique_ptr<i18n> i18nObject = SetupI18nObject(
                "unexisting_folder", SPANISH_SPAIN.data()),
...
{
    EXPECT_THROW(
            std::unique_ptr<i18n> i18nObject = SetupI18nObject(BASE_FOLDER,
...
{
    std::unique_ptr<I18nEnvironment> I18EnvironmentObject = std::make_unique<
        I18nEnvironment>();
    ...
    EXPECT_THROW(
            std::unique_ptr<i18n> i18nObject = SetupI18nObject(BASE_FOLDER,
                "xx_XX.UTF-8"), locale_exception);
}

TEST_F(i18nTests, BasicTranslation)
{
    auto i18nObject =
            SetupI18nObject(BASE_FOLDER, ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, TranslationWithParameters)
{
    ...
    i18nObject->addTranslation("welcome", "Welcome {name}",
                               ENGLISH_UNITED_STATES.data());
    i18nObject->addTranslation("welcome", "Bienvenido {name}",
                               SPANISH_SPAIN.data());
    ...
    result = i18nObject->translateWithParams("welcome", params,
                                             SPANISH_SPAIN.data());
...
TEST_F(i18nTests, GetAvailableLanguages)
{
    ...
    EXPECT_TRUE(
            find(languages.begin(), languages.end(), ENGLISH_UNITED_STATES.data(
            )) != languages.end());
    EXPECT_TRUE(
            find(languages.begin(), languages.end(), SPANISH_SPAIN.data()) !=
            languages.end());
}
...
TEST_F(i18nTests, GetTranslations)
{
    ...
    std::unordered_map<string, string> englishTranslations = i18nObject->
            getTranslations(ENGLISH_UNITED_STATES.data());
    std::unordered_map<string, string> spanishTranslations = i18nObject->
            getTranslations(SPANISH_SPAIN.data());
...
TEST_F(i18nTests, GetTranslationsForFallbackLanguage)
{
    ...
    const pair<const string, unordered_map<string, string> >*
            fallbackTranslations = i18nObject->
...
TEST_F(i18nTests, GetExistingLanguage)
{
    ...
    const pair<const string, unordered_map<string, string> >* spanishLanguage =
            i18nObject->getLanguage(
                    SPANISH_SPAIN.data());
...
TEST_F(i18nTests, loadTranslationFile_PropertiesFormat_Success)
{
    ...
    // Assuming German properties file exists
...
TEST_F(i18nTests, loadTranslationFile_PoFormat_Success)
{
    ...
    // Assuming French .po file exists
    EXPECT_TRUE(i18nObject->hasLanguage(FRENCH_FRANCE.data()));
...
TEST_F(i18nTests, loadTranslationFile_TxtFormat_Success)
{
    ...
    i18nObject->addLanguage(ITALIAN_ITALY.data());
    // Assuming Italian .txt file exists
...
TEST_F(i18nTests, loadTranslationFile_MultipleFormatsAvailable_LoadsFirst)
{
    ...
    EXPECT_THROW(
            i18nObject->addLanguage ("multi"),
            // Assuming corrupted JSON file exists
            ADS::i18n::locale_exception
            );
...
TEST_F(i18nTests, loadTranslationFile_CorruptedJsonFile_FallsBackToProperties)
{
    ...
    EXPECT_THROW(
            i18nObject->addLanguage ("corrupt_json"),
            // Assuming corrupted JSON file exists
            ADS::i18n::locale_exception
            );
...
TEST_F(i18nTests, loadTranslationFile_FileExistsButCannotOpen_ReturnsFalse)
{
    ...
    EXPECT_THROW(
            i18nObject->addLanguage ("restricted"),
...
TEST_F(i18nTests, loadTranslationFile_JsonWithNestedObjects_ParsesCorrectly)
{
    ...
    EXPECT_THROW(
            i18nObject->addLanguage ("nested_json"),
...
TEST_F(i18nTests,
       loadTranslationFile_PropertiesWithEscapeSequences_UnescapesCorrectly)
{
    auto i18nObject =
            SetupI18nObject(BASE_FOLDER, ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests,
       loadTranslationFile_PoFileWithMultilineStrings_ParsesCorrectly)
{
    auto i18nObject =
            SetupI18nObject(BASE_FOLDER, ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, getSupportedLocales)
{
    ...
    EXPECT_EQ(supportedLanguages.size(),
              ADS::Constants::Languages::languages.size());
    EXPECT_NE(std::find(
...
TEST_F(i18nTests, saveTranslations_ExistingPropertiesFile_Success)
{
    ...
    i18nObject->addTranslation("goodbye", "Auf Wiedersehen",
                               GERMAN_GERMANY.data());
...
TEST_F(i18nTests, saveTranslations_JsonFormat_ContainsMetadata)
{
    ...
    // Add some translations
    i18nObject->addTranslation("test_key", "test_value",
...
TEST_F(i18nTests, saveTranslations_TranslationsAreSorted)
{
    ...
    i18nObject->addTranslation("zebra", "zebra_translation",
                               ITALIAN_SWITZERLAND.data());
    i18nObject->addTranslation("apple", "apple_translation",
                               ITALIAN_SWITZERLAND.data());
    i18nObject->addTranslation("banana", "banana_translation",
                               ITALIAN_SWITZERLAND.data());
...
TEST_F(i18nTests, saveTranslations_SpecialCharactersEscaped)
{
    ...
    i18nObject->addTranslation("quotes", "He said \"Hello\"",
                               ROMANIAN_ROMANIA.data());
    i18nObject->addTranslation("newlines", "Line 1\nLine 2",
                               ROMANIAN_ROMANIA.data());
    i18nObject->addTranslation("tabs", "Column 1\tColumn 2",
                               ROMANIAN_ROMANIA.data());
...
TEST_F(i18nTests, saveTranslations_MultipleFormatsExist_SavesToFirst)
{
    ...
    i18nObject->addTranslation("multi_format", "test_value",
                               FRENCH_CANADA.data());
...
TEST_F(i18nTests, saveTranslations_Integration_SaveAndReload)
{
    ...
    auto newI18nObject = SetupI18nObject(BASE_FOLDER,
                                         ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, reloadTranslations_LanguagesWithoutFiles_CreatesEmptyMaps)
{
    ...
    i18nObject->addTranslation("manual_key", "manual_value",
                               PORTUGUESE_PORTUGAL.data());

    // Get count before reload
    auto beforeTranslations = i18nObject->getTranslations(
            PORTUGUESE_PORTUGAL.data());
    EXPECT_FALSE(beforeTranslations.empty());
    // Should have our manual translation

    // Reload translations
    size_t reloadedCount = i18nObject->reloadTranslations();

    // Manual translations should be gone (replaced with file content or empty)
    auto afterTranslations = i18nObject->getTranslations(
            PORTUGUESE_PORTUGAL.data());
...
TEST_F(i18nTests, reloadTranslations_ClearsManualTranslations)
{
    ...
    i18nObject->addTranslation("manual1", "Manual Translation 1",
                               SWEDISH_SWEDEN.data());
    i18nObject->addTranslation("manual2", "Manual Translation 2",
                               SWEDISH_SWEDEN.data());

    // Verify manual translations exist
    EXPECT_EQ(i18nObject->translate("manual1", SWEDISH_SWEDEN.data()),
              "Manual Translation 1");
    EXPECT_EQ(i18nObject->translate("manual2", SWEDISH_SWEDEN.data()),
...
TEST_F(i18nTests, reloadTranslations_Integration_SaveReloadCycle)
{
    ...
    EXPECT_EQ(i18nObject->translate("temp", testLang), "temp");
...
TEST_F(i18nTests, findMissingTranslations_TargetLanguageComplete_ReturnsEmpty)
{
    ...
    i18nObject->addTranslation("hello", "Hello", ENGLISH_UNITED_STATES.data());
    i18nObject->addTranslation("goodbye", "Goodbye",
                               ENGLISH_UNITED_STATES.data());
    i18nObject->addTranslation("thank_you", "Thank you",
                               ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, findMissingTranslations_TargetMissingSome_ReturnsCorrectList)
{
    ...
    i18nObject->addTranslation("thank_you", "Thank you",
                               ENGLISH_UNITED_STATES.data());
    i18nObject->addTranslation("please", "Please",
                               ENGLISH_UNITED_STATES.data());
    ...
    EXPECT_NE(std::find(missing.begin(), missing.end(), "thank_you"),
              missing.end());
    EXPECT_NE(std::find(missing.begin(), missing.end(), "please"),
              missing.end());
    EXPECT_NE(std::find(missing.begin(), missing.end(), "yes"), missing.end());

    // Should NOT contain the keys that exist
    EXPECT_EQ(std::find(missing.begin(), missing.end(), "hello"),
              missing.end());
    EXPECT_EQ(std::find(missing.begin(), missing.end(), "goodbye"),
              missing.end());
}

TEST_F(i18nTests,
       findMissingTranslations_TargetLanguageEmpty_ReturnsAllFallbackKeys)
{
    auto i18nObject =
            SetupI18nObject(BASE_FOLDER, ENGLISH_UNITED_STATES.data());

    // Add translations to fallback language
    std::vector<std::string> fallbackKeys = {"hello", "goodbye", "thank_you",
                                             "please", "welcome"};
    for (const auto& key: fallbackKeys) {
        i18nObject->addTranslation(key, "English_" + key,
                                   ENGLISH_UNITED_STATES.data());
    }
    ...
    for (const auto& key: missing) {
        EXPECT_NE(std::find(fallbackKeys.begin(), fallbackKeys.end(), key),
                  fallbackKeys.end());
    }
}
...
TEST_F(i18nTests,
       findMissingTranslations_FallbackLanguageNotLoaded_ReturnsEmpty)
{
    auto i18nObject =
            SetupI18nObject(BASE_FOLDER, ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, findMissingTranslations_ResultIsSorted)
{
    ...
    i18nObject->addTranslation("banana", "Banana",
                               ENGLISH_UNITED_STATES.data());
    i18nObject->addTranslation("cherry", "Cherry",
                               ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, findMissingTranslations_MultipleLanguages_IndependentResults)
{
    ...
    std::vector<std::string> allKeys = {"hello", "goodbye", "thank_you",
                                        "please", "yes", "no"};
    for (const auto& key: allKeys) {
        i18nObject->addTranslation(key, "EN_" + key,
                                   ENGLISH_UNITED_STATES.data());
    }
    ...
    auto spanishMissing = i18nObject->findMissingTranslations(
            SPANISH_SPAIN.data());
    EXPECT_EQ(spanishMissing.size(), 2);
    EXPECT_NE(std::find(spanishMissing.begin(), spanishMissing.end(), "yes"),
              spanishMissing.end());
    EXPECT_NE(std::find(spanishMissing.begin(), spanishMissing.end(), "no"),
              spanishMissing.end());

    // Check French missing translations
    auto frenchMissing = i18nObject->findMissingTranslations(
            FRENCH_FRANCE.data());
    EXPECT_EQ(frenchMissing.size(), 4);

    // Check German missing translations. two comes from the translation file
    auto germanMissing = i18nObject->findMissingTranslations(
            GERMAN_GERMANY.data());
...
TEST_F(i18nTests, findMissingTranslations_SameKeyDifferentValues_NotMissing)
{
    ...
    i18nObject->addTranslation("greeting", "Hello",
                               ENGLISH_UNITED_STATES.data());
    i18nObject->addTranslation("farewell", "Goodbye",
                               ENGLISH_UNITED_STATES.data());
...
TEST_F(i18nTests, findMissingTranslations_EmptyKeyValues_HandledCorrectly)
{
    ...
    EXPECT_NE(std::find(missing.begin(), missing.end(), "empty_key"),
              missing.end());
}
...
TEST_F(i18nTests, findMissingTranslations_Integration_RealWorldScenario)
{
    ...
    i18nObject->addTranslation("button.cancel", "Cancelar",
                               SPANISH_SPAIN.data());
    ...
    for (const auto& expectedKey: expectedMissing) {
        EXPECT_NE(std::find(missing.begin(), missing.end(), expectedKey),
                  missing.end())
    ...
    for (const auto& missingKey: missing) {
        EXPECT_NE(
                std::find(expectedMissing.begin(), expectedMissing.end(),
                    missingKey),
                expectedMissing.end()) << "Unexpected missing key: " <<
 missingKey;
    }
}
